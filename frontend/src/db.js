...
// Version 3 : toutes les tables finales
db.version(3).stores({
  ...
  stock_movements: 'id, product_id, cooperant_id, movement_type, created_at, quantity_change',
  ...
  sync_queue: '++id, synced'   // file d’attente des opérations hors ligne
}).upgrade(tx => {
  // Migration : initialiser les paramètres par défaut si la table est vide
...
export const clearAll = async () => {
  ...
  // Mais on peut la vider si nécessaire (ex: après un échec de sync)
};
